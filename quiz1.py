...
def display_header(chapter_selection):
    ...
    elif chapter_selection == "4":
        print("Chapter 4 Quiz - Fall 2024")
    else:
        print("Chapters 1 & 2 & 3 & 4 Combined Quiz - Fall 2024")
    print("=" * 80)
...
def merge_questions(chapter1_q, chapter2_q, chapter3_q, chapter4_q):
    """Merge questions from all three chapters"""
    merged = {}
    all_sections = set(list(chapter1_q.keys()) + list(chapter2_q.keys()) + list(chapter3_q.keys()) + list(chapter4_q.keys()))
    
    for section in all_sections:
        # Get title from first available chapter
        title = None
        if section in chapter1_q:
            title = chapter1_q[section]["title"]
        elif section in chapter2_q:
            title = chapter2_q[section]["title"]
        elif section in chapter3_q:
            title = chapter3_q[section]["title"]
        elif section in chapter4_q:
            title = chapter4_q[section]["title"]
        ...
        # Add chapter 2 questions with offset
        ...
        # Add chapter 3 questions with offset
        if section in chapter3_q:
            offset = len(merged[section]["questions"])
            for q in chapter3_q[section]["questions"]:
                q_copy = q.copy()
                q_copy["num"] = q["num"] + offset

                merged[section]["questions"].append(q_copy)
        # Add chapter 4 questions with offset
        if section in chapter4_q:
            offset = len(merged[section]["questions"])
            for q in chapter4_q[section]["questions"]:
                q_copy = q.copy()
                q_copy["num"] = q["num"] + offset
                merged[section]["questions"].append(q_copy)
...
def run_quiz():
    ...
    if chapter_selection == "1":
        ...
        questions = get_chapter_4_questions()
    else:
        chapter1_q = get_chapter_1_questions()
        chapter2_q = get_chapter_2_questions()
        chapter3_q = get_chapter_3_questions()
        chapter4_q = get_chapter_4_questions()
        questions = merge_questions(chapter1_q, chapter2_q, chapter3_q, chapter4_q)
